...
extern String opah;
namespace AX25 {
//此為 LoRa APRS iGate 修正版，可以正常顯示 Digi 路徑呼號 By BX2AJD
String opah = "";
...
{
  String txt = srcCall_.ToString() + String(">");
 
  if(opah != false or opah != "") {
...
bool Payload::Digirepeat(const Callsign &ownCallsign)
{
  for (int i = 0; i < rptCallsCount_; i++) {   
    if (rptCalls_[i].Digirepeat()) {
      ...
      if (rptCalls_[i].IsWide()) {          //modified 07/11
...
bool Payload::fromString(const String &textPayload)
{
  ...
  String inputTextO = textPayload;
  
  int rptIndex = inputText.indexOf('>');
  int infoIndex = inputText.indexOf(':');
  opah = inputTextO.substring(inputText.indexOf('>') + 1,inputText.indexOf(':'));
